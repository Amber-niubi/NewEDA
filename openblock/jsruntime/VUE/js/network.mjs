...
class OBNetworkImpl {
    ...
    onPeerJoin(name) {
        let j = new Network_target(name);
        this.joined_peers[name] = j;
        this.obvm.BroadcastMessage(new obvm.EventMessage('network_peer_join', 'Network_target', j, null));
    }
    setConnector(c) {
        this.connector = c;
        c.setNetwork(this);
    ...
    onPeerLeave(name) {
        let j = this.joined_peers[name];
        delete this.joined_peers[name];
        if (j) {
            this.obvm.BroadcastMessage(new obvm.EventMessage('network_peer_leave', 'Network_target', j, null));
        }
    }
    join(obvm) {
...
function send_message_install(builder, args) {
    ...
    builder.PushAction((st, f, local, pos) => {
        ...
        that.impl.sendMessage(target.value, 0, msg, st, f, local);
        return pos + 1;
    });
}
class OBNetwork {
