...
function syncDebuggerSetting() {
    ...
    if (vmdebugger) {
        ...
        vm.logLevel = Math.min(usrLogLevel, sysLogLevel);
    }
}
/**
 * 检测长时间运行
...
window.runProject = function (projData) {
    ...
    for (let name in _assets) {
        ...
        if (!found) {
            for (let suffixs in textsuffixs) {
                if (lname.endsWith(textsuffixs[suffixs])) {
                    assets[name] = (new util.TextDecoder()).decode(_assets[name]);
                    found = true;
                    break;
                }
            }
...
if (window.parent) {
    let messageHandler = {
        ...
        requestFullScreen(evt) {
            ...
            } else if (stage.mozRequestFullScreen) { // Firefox  
                stage.mozRequestFullScreen();
            } else if (stage.webkitRequestFullscreen) { // Chrome, Safari and Opera  
                stage.webkitRequestFullscreen();
            } else if (stage.msRequestFullscreen) { // IE/Edge  
...
if (location.search) {
    ...
    if (p) {
        ...
        //     PROJ_URL = PROJ_URL + s + '/'
        // }
        const LIST_FILE_URL = s ? PROJ_URL + s + '/' + LIST_FILE : PROJ_URL + LIST_FILE;
        let mainXE;
