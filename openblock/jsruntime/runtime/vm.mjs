    ...
    static createInstruction(cmd) {
        switch (cmd) {
            ...
            case 84:
                return new ENVREF();
            case 85:
                return new FSMSendMessageIntervalToSelf();
            default:
                throw Error("Unknown byte code command:" + cmd);
        }
...
class OBBuildInFunctions {
    ...
    static install(script) {
        script.InstallLib("", "", [
            ...
            script.NativeUtil.closureReturnValue(OBBuildInFunctions.Text_IndexOfFrom, 'LongRegister', ['StringRegister', 'StringRegister', 'LongRegister']),
        ]);
    }
    ...
    static Text_IndexOfFrom(str, sub, from) {
...
class StructData {
    ...
    Count(type) {
        ...
        if (!group) {
            console.warn(`There is no preset data of the type ${type}`);
            return 0;
        }
...
class ScriptLoader {
    ...
    readXE() {
        ...
        let headerEnd = this.reader.pos + SegmentCnt * 2 * 4 + 4;
        ...
        let entryFSMIdx = this.reader.ReadUInt32();
        this.script.entryFSMName = this.data.GetString(entryFSMIdx);
    ...
    readFSM(reader) {
        ...
        let functions = this.readFunctions(reader);
    ...
    readVariable(reader) {
        ...
        for (let i = 0; i < count; i++) {
            let nameIdx = reader.ReadUInt32();
            let name = this.data.GetString(nameIdx);
            let tIdx = reader.ReadUInt32();
            let type = this.data.GetString(tIdx);
            v.info.push({ name, type });
        }
        return v;
    }
...
class VM extends EventTarget {
    ...
    constructor(script, config) {
        ...
        if (!StartEvent) {
            StartEvent = new EventMessage("Start", "", null, null);
            RestoreEvent = new EventMessage("Restore", null, null);
        }
        this.logLevel = 5;
        if (config) {
    ...
    */
    buildEventMessage(name, argType, arg, sender) {
        return new EventMessage(name, argType, arg, sender);
    }
    /**
    *
    * @param {string} name
    ...
    * @param {?OBVMFSM} sender
    */
    buildUsertMessage(name, argType, arg, sender) {
        return new UserMessage(name, argType, arg, sender);
    }
    ...
    CreateFSM(name, st, ofun) {
        ...
        if (uBFSM.CurrentState.data.EventHandlers.Start) {
            uBFSM.PostPrioritizedMessage(StartEvent);
        }
        ...
        this.dispatchEvent(new CustomEvent('FSMCreated', {
            detail: {
                fsm: uBFSM,
                vm: this,
                stacks: VM.stackInfo(uBFSM.CurrentState),
                name
            }
        }));
        return uBFSM;
            ...
            if (idx > -1) {
                ...
                this.dispatchEvent(new CustomEvent('FSMCreated', {
                    detail: {
                        fsm,
                        vm: this,
                        stacks: VM.stackInfo(fsm.CurrentState),
                        name
                    }
                }));
            }
    ...
    /**
         ...
         */
    BroadcastMessage(message, sendToSelf) {
        if (message instanceof EventMessage) {
            let name = message.name;
            let set = this.eventListener.get(name);
            if (set) {
                set.forEach((_, fsm) => {
            ...
            Object.values(this.Running).forEach(l => {
                for (let i = 0; i < l.length; i++) {
                    let f = l[i];
                    if (f && (sendToSelf || f != message.sender)) {
...
let ID_GEN = 1;
class OBVMFSM extends EventTarget {
    // /**
    //  * @type {any}
    //  */
    // Target;
    ...
    getPropValue(propName) {
        for (let i = 0; i < 5; i++) {
            let arr = this.data.Variables[i].info;
            let p = arr.findIndex(v => v.name == propName);
            if (p > -1) {
                switch (i) {
                    case 0:
                        return this.VariableGroup.LongRegister[p];
                    case 1:
                        return this.VariableGroup.DoubleRegister[p];
                }
            }
        }
        return 0;
    ...
    Destroy() {
        ...
        let e = new CustomEvent('FSMDestroyed', {
            detail: {
                fsm: this,
                vm: this.VM
            }
    ...
    ChangeState(name, ofun) {
        ...
        if (this.data.States[name]) {
            ...
            this.CurrentState.mounted();
            if (this.CurrentState.data.EventHandlers.Start) {
                this.PostPrioritizedMessage(StartEvent);
            }
            if (this.VM.debugger) {
                let args = {
    ...
    PopState(f) {
        ...
        this.PostPrioritizedMessage(RestoreEvent);
        if (this.VM.debugger) {
            let args = {
                lastState: lastState.data.Name,
...
class OBVMState {
    ...
    mounted() {
        let fsm = this.fsm;
        let vm = fsm.VM;
        let l = vm.eventListener;
        Object.keys(this.data.EventHandlers).forEach(name => {
            let lst = l.get(name);
            if (!lst) {
                lst = new Map();
                l.set(name, lst);
...
class OBVMFunction {
    ...
    constructor(obfunc, builder, args) {
        ...
        let funcArgs = this.data.Signure.substring(this.data.Signure.indexOf('(') + 1, this.data.Signure.indexOf(')')).split(',');
        ...
        if (builder) {
            ...
            for (let i = 1; i < args.length; i++) {
                ...
                switch (RegisterType) {
                    case 0:
                        if (funcArgs[i - 1] == 'SNumber;') {
                            if (!DoubleRegister_) {
                                DoubleRegister_ = [];
                            }
                            DoubleRegister_.push(builder.LongRegister(Register));
                        } else {
                            if (!LongRegister_) {
                                LongRegister_ = [];
                            }
    ...
    link(builder, instructions, idx) {
        ...
        switch (RegisterType) {
            case 0:
                let fl = builder.LongRegister(Register);
                builder.PushAction((st, uf, locals, pos) => {
                    let info = st.fsm.data.Variables[0].info[VarIdx];
                    let name = info.name;
                    let type = info.type;
                    let oldValue = st.fsm.VariableGroup.LongRegister[VarIdx];
                    let newValue = fl(st, uf, locals);
                    st.fsm.VariableGroup.LongRegister[VarIdx] = newValue;
                    {
                        st.fsm.dispatchEvent(new CustomEvent('FSMVarChanged', {
                            detail: {
                                oldValue,
                                newValue,
                                fsm: st.fsm,
            ...
            case 1:
                ...
                builder.PushAction((st, uf, locals, pos) => {
                    ...
                    let oldValue = st.fsm.VariableGroup.DoubleRegister[VarIdx];
                    let newValue = fd(st, uf, locals);
                    let info = st.fsm.data.Variables[1].info[VarIdx];
                    let name = info.name;
                    let type = info.type;
                    st.fsm.VariableGroup.DoubleRegister[VarIdx] = newValue;
                    {
                        st.fsm.dispatchEvent(new CustomEvent('FSMVarChanged', {
                            detail: {
                                oldValue,
            ...
            case 2:
                let fs = builder.StringRegister(Register);
                builder.PushAction((st, uf, locals, pos) => {
                    let info = st.fsm.data.Variables[2].info[VarIdx];
                    let name = info.name;
                    let type = info.type;
                    let oldValue = st.fsm.VariableGroup.StringRegister[VarIdx];
                    let newValue = fs(st, uf, locals);
                    st.fsm.VariableGroup.StringRegister[VarIdx] = newValue;
                    {
                        st.fsm.dispatchEvent(new CustomEvent('FSMVarChanged', {
                            detail: {
                                oldValue,
            ...
            case 3:
                let fst = builder.StructRegister(Register);
                builder.PushAction((st, uf, locals, pos) => {
                    let info = st.fsm.data.Variables[3].info[VarIdx];
                    let name = info.name;
                    let type = info.type;
                    let oldValue = st.fsm.VariableGroup.StructRegister[VarIdx];
                    ...
                    let newValue = v;
                    st.fsm.VariableGroup.StructRegister[VarIdx] = newValue;
                    {
                        st.fsm.dispatchEvent(new CustomEvent('FSMVarChanged', {
                            detail: {
                                oldValue,
            ...
            case 4:
                let fo = builder.NObjectRegister(Register);
                builder.PushAction((st, uf, locals, pos) => {
                    let info = st.fsm.data.Variables[4].info[VarIdx];
                    let name = info.name;
                    let type = info.type;
                    let oldValue = st.fsm.VariableGroup.NObjectRegister[VarIdx];
                    let newValue = fo(st, uf, locals);
                    st.fsm.VariableGroup.NObjectRegister[VarIdx] = newValue;
                    {
                        st.fsm.dispatchEvent(new CustomEvent('FSMVarChanged', {
                            detail: {
                                oldValue,
    ...
    link(builder, instructions, idx) {
        ...
        builder.PushAction((st, uf, locals, pos) => {
            ...
            if (fsm) {
                ...
                fsm.PostMessage(new UserMessage(title, this.TypeName || Message.ArgTypeOf(this.BodyTypeID, body), body, st.fsm));
            }
            return ++pos;
        });
    }
    ...
    link(builder, instructions, idx) {
        ...
        switch (this.type) {
            ...
            case 4:
                ...
                builder.PushAction((st, uf, locals, pos) => {
                    let v = getNObject(st, uf, locals);
                    locals.NObjectRegister[varIdx] = v;
                    return 1 + pos;
                });
                break;
            default:
...
class FSMSendMsgWait extends OBInstruction {
    ...
    init(code, builder, instructions, i) {
        ...
        this.argTypeNameIdx = code & 0x1ffff;
    }
    ...
    link(builder, instructions, idx) {
        ...
        builder.PushAction((st, uf, locals, pos) => {
            ...
            fsm.VM.Schedule(waitSecond, (VM) => {
                fsm.PostMessage(new UserMessage(title,
                    this.argTypeNameIdx ? builder.loader.data.GetString(this.argTypeNameIdx) : null
                    , body, FSM));
            });
            return ++pos;
        });
...
class FSMSendMessageIntervalToSelf extends OBInstruction {
    // TitleIdx;
    // BodyTypeID;
    // BodyRegisterIdx;
    // waitMilliSecondIdx;
    ...
    link(builder, instructions, idx) {
        ...
        builder.PushAction((st, uf, locals, pos) => {
            let title = f_title(st, uf, locals);
            let waitMilliSecond = waitTime(st, uf, locals);
            let fsm = st.fsm;
            function send(VM) {
                if (fsm.Running && fsm.CurrentState == st) {
                    fsm.VM.Schedule(waitMilliSecond, send);
                    fsm.PostMessage(fsm.PostMessage(new UserMessage(title, null, null, fsm)));
                }
            }
            fsm.VM.Schedule(waitMilliSecond, send);
            return ++pos;
    ...
    link(builder, instructions, idx) {
        ...
        arr.call(builder, this.RetRegIdx, (st, f, l) => {
            ...
            if (typeof (v) === 'undefined') {
                let msg = 'Read non-existent key: ' + k + ' of list length ' + m.c.length;
                console.error(msg);
                throw VM.makeError(msg, st, f, l);
            }
            return v;
        });
    }
    ...
    link(builder, instructions, idx) {
        ...
        arr.call(builder, this.RetRegIdx, (st, f, l) => {
            ...
            if (typeof (v) === 'undefined') {
                ...
                let msg = 'Read non-existent key: ' + k;
                console.error(msg);
                throw VM.makeError(msg, st, f, l);
            }
            return v;
...
class ENVREF extends OBInstruction {
    // Value;
    // Register;
