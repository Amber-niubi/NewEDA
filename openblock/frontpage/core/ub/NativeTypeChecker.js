...
class NativeTypeChecker extends Blockly.ConnectionChecker {
  ...
  doTypeChecks(a, b) {
    var checkArrayOne = a.getCheck();
    var checkArrayTwo = b.getCheck();
    let workspace = a.getSourceBlock().workspace;
    let src = workspace._openblock_env._openblock_src;
    let env = src.env;
    if (!checkArrayOne || !checkArrayTwo) {
      // One or both sides are promiscuous enough that anything will fit.
      return true;
    ...
    if (a.type === Blockly.connectionTypes.INPUT_VALUE) {
      return this.NativeTypeChecker(checkArrayOne, checkArrayTwo, env);
    } else if (a.type === Blockly.connectionTypes.OUTPUT_VALUE) {
      return this.NativeTypeChecker(checkArrayTwo, checkArrayOne, env);
    }
    return false;

  }
  NativeTypeChecker(inputArr, outputArr, env) {
    for (let o = 0; o < outputArr.length; o++) {
      let output = outputArr[o];
      for (let i = 0; i < inputArr.length; i++) {
        let input = inputArr[i];
        if (this.match(output, input, env)) {
          return true;
        }
      }
    }
  ...
  match(sub, parent, env) {
    ...
    let types = OpenBlock.Env.getNativeTypesWithParent(env);
    let parents = types[sub];
    if (parents) {
      for (let i = 0; i < parents.length; i++) {
        let p = parents[i];
        if (this.match(p, parent, env)) {
          subMath[parent] = true;
          return true;
        }
      }
