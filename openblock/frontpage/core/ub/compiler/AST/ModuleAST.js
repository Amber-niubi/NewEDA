...
class StructFieldTypeAny extends StructFieldType {
    registerType() {
        return 'Any';
    }
    toCodeText() {
...
class BooleanExpr extends IntegerExpr {
    constructor() {
        super();
        this.astType = 'Boolean';
    }
}
Serializable(BooleanExpr);
class BooleanConstExpr extends IntegerConstExpr {
    constructor() {
        super();
        this.astType = 'Boolean';
    }
}
Serializable(BooleanConstExpr);
...
class FunctionDef extends Definition {
    ...
    buildSignature() {
        let signature = this.fullname + '(' + this.args.map(a => { return a.type ? a.type.compile() : new StructFieldTypeAny() }).join() + ')' + (this.returnType ? this.returnType.compile() : "v");
...
class FSMBroadcastMessageWait extends Instruction {
    ...
    setMillisecond(expr) {
        ...
        this.waitMillisecond = expr;
    }
}
Serializable(FSMBroadcastMessageWait);
class FSMSendMessageIntervalToSelf extends Instruction {
    title;
    intervalMillisecond;
    setTitle(t) {
        if (!(t instanceof TextConstExpr)) {
    ...
    setMillisecond(expr) {
        ...
        this.intervalMillisecond = expr;
    }
}
Serializable(FSMSendMessageIntervalToSelf);
...
class EnvRef extends ValueExpr {
    value; type;
...
class Modulo extends IntegerExpr {
    dividend; divisor;
    constructor(d1, d2) {
        super();
        this.dividend = d1;
