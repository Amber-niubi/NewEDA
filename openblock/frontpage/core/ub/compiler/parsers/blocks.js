...
OpenBlock.BlocklyParser.BlockToAST = (function () {
    ...
    return {
        ...
        math_trig_atan2(m) {
            ...
            let ast = new ARITH();
            ast.setOP(op);
            ast.setLeft(BlocktoExpr(m.values.Y, "必须提供值"));
            ast.setRight(BlocktoExpr(m.values.X, "必须提供值"));
            ast.setBlockId(m.id);
            return ast;
        },
        math_single(block) {
        ...
        native_call(block) {
            ...
            let func = OpenBlock.Env.matchNativeFunction(block.fsm.src.env, mutationData.func, true);
            let ast;
        ...
        fsm_send_interval_message_self(block) {
            ...
            ast.setTitle(new TextConstExpr(block.fields.TITLE.text));
            ast.setMillisecond(BlocktoExpr(block.values.INTERVAL_MILLISECOND));
            ast.setBlockId(block.id);
            return ast;
        },
        fsm_broadcast_message_after_millisecond(block) {
            ...
            ast.setTitle(new TextConstExpr(block.fields.TITLE.text));
            ast.setMillisecond(BlocktoExpr(block.values.WAIT_MILLISECOND));
            if (block.values.ARG) {
                ast.setBodyExpr(BlocktoExpr(block.values.ARG));
            }
        ...
        integer_to_colour(block) {
            ...
            let v = BlocktoExpr(block.values.value);
            return v;
        },
