        ...
        if (code instanceof VoidMethodCall ||
            ...
            if (code.MethodName) {
                ...
                if (targetFullname.startsWith('../')) {
                    targetFullname = targetFullname.substring(3);
                    targetFullname = this.currentModule.name
                        + (this.currentFSM ? '.' + this.currentFSM.name + '.' : '')
                        + targetFullname;
                } else if (targetFullname.startsWith('.')) {
                    targetFullname = this.currentModule.name
                        + (this.currentFSM ? '.' + this.currentFSM.name + '.' + (this.currentState ? this.currentState.name : '') : '')
                        + targetFullname;
                }
            } else {
