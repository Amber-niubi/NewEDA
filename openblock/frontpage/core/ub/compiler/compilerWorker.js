...
class Instruct extends Data {
    ...
    constructor(ast, ctx, fCtx) {
        ...
        this.code = this.constructor.CODE >> 24;
...
class RegisterHandler {
    ...
    sn;
    astType;
    constructor(arr, i, rtype, astType) {
        this.arr = arr;
        this.i = i;
        this.rtype = rtype;
        if (astType) {
...
class CompileContext {
    ...
    pushData(data) {
        this.data.push(data);
        if (this.currentCompileUnit) {
            this.currentCompileUnit.data.data.push(data);
        }
    }
    setCompileUnit(u) {
        let data = { from: this.data.length, data: [] };
        let s = u.funcDef.type + '_' + u.funcDef.signature;
        this.compileUnits[s] = data;
        this.currentCompileUnit = { unit: u, data, };
    }
    clearCompileUnit() {
        this.currentCompileUnit = null;
    }
    getWritingDataIdx() {
...
self.onmessage = function (msg) {
    ...
    try {
        ...
        let cu = JSON.parse(JSON.stringify(ctx.compileUnits, (k, v) => {
            if (v instanceof CompileContext) {
            }
            else if (v instanceof RegisterHandler) {
                delete v.arr;
                delete v.caller;
                return v;
            }
            else if (v && v.constructor && v.constructor.name == 'FunctionContext') {
            }
            else if (v && v.constructor && v.constructor.name == 'ModuleDef') {
            }
            else if (v && v.constructor && v.constructor.name == 'FSMDef') {
            }
            else if (v && v.constructor && v.constructor.name == 'StateDef') {
            }
            else if (v && v.constructor && v.constructor.name == 'StructDef') {
            }
            else {
                return v;
            }
