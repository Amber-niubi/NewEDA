...
(function () {
    function astType2BitcodeType(astType) {
        ...
        if (OpenBlock.nativeTypes.includes(astType)) {
            return "Nobject";
        }
        switch (astType) {
            case 'Number':
    ...
    function pushVariables(info, ctx) {
        ...
        keys.forEach(rtype => {
            ...
            arr.forEach(vari => {
                let idx = ctx.buildRelocationInfo('string', vari.name);
                let pos = ctx.getWritingDataIdx();
                idx.idx = pos;
                ctx.pushData(idx);
                //
                let tidx = ctx.buildRelocationInfo('string', vari.type);
                let tpos = ctx.getWritingDataIdx();
                tidx.idx = tpos;
                ctx.pushData(tidx);
            });
        });
    ...
    ModuleDef.prototype.compile = function (ctx) {
        ...
        this.fsms.forEach(fsm => {
            ...
            if (ctx.export.fsms[fullname]) {
                ctx.markError(f.blockId, ctx, `${OpenBlock.i('FSM')}类型重名:` + f.name);
            }
    ...
    class FunctionContext {
        ...
        createLocalVar(type, decl) {
            ...
            for (let i = 0; i < arr.length; i++) {
                if (!arr[i]) {
                    ...
                    arr[i] = r;
                    if (decl && decl.name) {
                        this.namedLocalVariables[decl.name.trim()] = { register: r, decl: decl };
                    }
                    return r;
                }
            }
        ...
        getRegister(type, astType) {
            ...
            for (let i = 0; i < arr.length; i++) {
                if (!arr[i]) {
                    let r = new RegisterHandler(arr, i, type, astType);
                    r.sn = sn;
            ...
            let r = new RegisterHandler(arr, arr.length, type, astType);
            r.sn = sn;
            r.caller = new Error().stack;
            arr.push(r);
            return r;
    ...
    Compare.prototype.compile = function (ctx, fCtx) {
        ...
        switch (this.op) {
            case "EQ":
                if (leftInfo.is('number') || leftInfo.is('integer')) {
    ...
    IntegerConstExpr.prototype.compile = function (ctx, fCtx) {
        ctx.analyser.visitCode(this, ctx, fCtx);
        let r = fCtx.getRegister('integer', this.astType);
    ...
    ChangeState.prototype.compile = function (ctx, fCtx) {
        ...
        idx.idx = pos;
        if (!ctx.currentFSM) {
            ctx.markError(this.blockId, fCtx, `切换状态指令只能用于${OpenBlock.i('FSM')}作用域`);
        }
    ...
    function writeMethodCall(ctx, fCtx, register) {
        if (this.MethodName.startsWith('.')) {
            ...
            let simpleName;
            if (this.MethodName.startsWith('../')) {
                scope = ctx.currentFSM;
                simpleName = this.MethodName.substring(3);
            } else {
                scope = ctx.currentState || ctx.currentFunctions || ctx.currentModule;
                simpleName = this.MethodName.substring(1);
            }
            if (scope) {
    ...
    FSMSendMessageWait.prototype.compile = function (ctx, fCtx) {
        ...
        let code1 = new B_FSMSendMsgWait(this, ctx, fCtx, wait, typeNameIdx);
        ctx.pushData(code1);
        target.return();
        title.return();
        wait.return();
    ...
    FSMBroadcastMessageWait.prototype.compile = function (ctx, fCtx) {
        ...
        let title = this.title.compile(ctx, fCtx);
        let bodyExpr = this.bodyExpr ? this.bodyExpr.compile(ctx, fCtx) : null;
        let wait = this.waitMillisecond.compile(ctx, fCtx);
    ...
    FSMSendMessageIntervalToSelf.prototype.compile = function (ctx, fCtx) {
        ctx.analyser.visitCode(this, ctx, fCtx);
        let title = this.title.compile(ctx, fCtx);
        let wait = this.intervalMillisecond.compile(ctx, fCtx);
        let code = new B_FSMSendMessageIntervalToSelf(this, ctx, fCtx, title, wait);
        ctx.pushData(code);
        title.return();
        wait.return();
        ctx.analyser.visitCodeEnd(this, ctx, fCtx);
    ...
    Modulo.prototype.compile = function (ctx, fCtx) {
        ...
        let b = new B_ARITHI(this, ctx, fCtx, dividend, divisor, 5);
        ctx.pushData(b);
        divisor.return();
    ...
    EnvRef.prototype.compile = function (ctx, fCtx) {
        ctx.analyser.visitCode(this, ctx, fCtx);
        let regType = astType2BitcodeType(this.type);
        let r = fCtx.getRegister(regType, this.type);
        let idx = ctx.buildRelocationInfo('string', this.value);
        let pos = ctx.getWritingDataIdx();
        idx.idx = pos;

        ctx.pushData(new B_ENVREF(this, ctx, fCtx, idx, r));
        return ctx.analyser.visitCodeEnd(this, ctx, fCtx, r);
    };
})();
