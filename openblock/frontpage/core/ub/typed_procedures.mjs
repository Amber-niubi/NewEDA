...
Blockly.Blocks["typed_procedure_call"] = {
    init() {
        ...
        this.appendDummyInput('BASE')
            .appendField(Blockly.Msg['调用函数'] || '调用函数')
            .appendField(Blockly.Msg['模块:'] || '模块:')
            .appendField(new Blockly.FieldDropdown(this.moudleList, this.moudleValidator), 'MODULE')
            .appendField(Blockly.Msg['函数:'] || '函数:')
    ...
    updateBlock() {
        ...
        while (block.inputList.length > this.mutationData.args.length + 1) {
            ...
            let cnt = block.inputList.length;
            block.removeInput(input.name);
            if (cnt == block.inputList.length) {
                break;
            }
        }
    ...
    funcList() {
        ...
        if (block.module === '..') {
            ...
            let module = OpenBlock.BlocklyParser.loadedFiles.
                srcs.find(m => m.name === src.name);
            let a_fsm = module.__analyzed.fsms.find(f => f.name == fsm.name);
            a_fsm.functions.forEach(f => {
                rt.push([f.name, '../' + f.name, f]);
            });
            if (rt.length == 0) {
                rt.push(['', '']);
            }
            return rt;
        } else if (block.module) {
            let module = OpenBlock.BlocklyParser.loadedFiles.
    ...
    funcValidator(v) {
        ...
        if (!func) {
            block.setWarningText(OpenBlock.i('找不到函数'));
        } else {
            this.func = func;
    ...
    moudleList() {
        let i18n_current = OpenBlock.i('当前作用域');
        let i18n_action = OpenBlock.i('行为');
        ...
        let src = env._openblock_src;
        let scenes = SceneManager.scenesWithSrc(src.name);
        let lst = OpenBlock.BlocklyParser.loadedFiles.dependingTree.availableModule(src.name);
        let lst1 = SceneManager.filterSameSceneWith(scenes.map(s => s.id), lst);
        let rt = [[i18n_current, '']];
        if (env._openblock_fsm) {
            rt.push([i18n_action, '..']);
        }
        rt.push([src.name, src.name]);
        lst1.forEach(m => {
            rt.push([m, m]);
        });
        return rt;
