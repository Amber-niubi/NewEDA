...
let searchblockInstance;
let searchblock = defineAsyncComponent(() => {
        ...
        axios.get('js/htmls/searchblock/searchblock.html').then(({ data }) => {
            resolve({
                ...
                methods: {
                    onVisibleChange(v) {
                        ...
                        if (!v) {
                            this.blocks = [];
                            setTimeout(() => {
                                OB_IDE.removeExtComponent('subwindows', searchblock);
                            }, 0);
                        }
                    }
                },
                watch: {
                    show(v) {
                        if (!v) {
                            OB_IDE.removeComponent(searchblockInstance);
                            searchblockInstance = null;
                        }
                    }
                },
                mounted() {
...
class searchblockComponent {
    ...
    search(text) {
        ...
        if (!text) {
            ...
            let matched = [];
            allblocks.forEach(b => {
                if (b.id.toLocaleLowerCase().indexOf(text) >= 0) {
                    matched.push(b);
                } else if (b.type.toLocaleLowerCase().indexOf(text) >= 0) {
                    matched.push(b);
                } else if (b.toString().toLocaleLowerCase().indexOf(text) >= 0) {
                    matched.push(b);
                }
            });
    ...
    async onClick_() {
        ...
        ui_data.show = true;
        if (!searchblockInstance) {
            searchblockInstance = await OB_IDE.addComponent(searchblock);
        }
    }
