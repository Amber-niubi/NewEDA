...
(function () {
    ...
    function loadHelpDoc(type) {
        // 设置为true以防止连续请求时一直加载
        Help.docCache[type] = true;
        let pathIdx = 0;
        function r() {
            if (pathIdx >= Help.docPath.length) {
                Help.docCache[type] = false;
                return;
            }
    ...
    Help.openHelpWindow = function (block_type) {
        ...
        Help.enabled = true;
        OB_IDE.ensureExtComponent('subwindows', openblockHelp);
    };
    var reader = new commonmark.Parser();
    var writer = new commonmark.HtmlRenderer();

    let openblockHelp = defineAsyncComponent(() => {
        ...
        Blockly.WorkspaceSvg.prototype.newBlock = function (prototypeName, opt_id) {
            ...
            if (OpenBlock.BlocklyParser.BlockToAST[prototypeName]) {
                if (nb.helpUrl && !Help.blocklyHelpUrl[prototypeName]) {
                    let url = nb.helpUrl;
                    Help.blocklyHelpUrl[prototypeName] = url;
                }
                nb.setHelpUrl();
            }
            return nb;
